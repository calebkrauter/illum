            ...
            {/* FOR testing... */}
            {
                globalCardContent.projects.length > 0 && (
                    <div className={`grid grid-cols-[70%_30%] grid-rows-[30%_70%] borderborder-gray-500 fixed inset-0 items-center justify-center z-10 ${className} ${isOpen ? 'bg-opacity-100' : 'bg-opacity-0'}`}>
                        <TextCard className={`bg-gray-700 ${isOpen ? 'bg-opacity-100' : 'bg-opacity-0 border-transparent text-transparent'}`} title={globalCardContent.projects[0].content.image.title} text={globalCardContent.projects[0].content.image.path}></TextCard>
                        <ListCard className={`bg-gray-700 ${isOpen ? 'bg-opacity-100' : 'bg-opacity-0 border-transparent text-transparent'}`} title={globalCardContent.projects[0].content.summary.title} text={globalCardContent.projects[0].content.summary.text || 'No summary available'} delimiter='*'></ListCard>
                        <TextCard className={`bg-gray-700 ${isOpen ? 'bg-opacity-100' : 'bg-opacity-0 border-transparent text-transparent'}`} title={globalCardContent.projects[0].content.description.title} text={globalCardContent.projects[0].content.description.text}></TextCard>
                        <ListCard className={`bg-gray-700 ${isOpen ? 'bg-opacity-100' : 'bg-opacity-0 border-transparent text-transparent'}`} title={globalCardContent.projects[0].content.stack.title} text={globalCardContent.projects[0].content.stack.text} delimiter='*'></ListCard>
                    </div>
                )
            }
